...
Admixtools is a set of programs used to infer population histories from genetic data. This implementation divides the computation into two steps: The first step performs most of the expensive computations which are necessary to run analyses on a given set of samples. This results in statistics which are stored on disk and which can be used in the second step for any further analyses on those samples.

This package can either be run through the `R` command line, or through a browser application that can be launched like this:
