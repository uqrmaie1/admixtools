...
$f_2$ is the average squared difference in allele frequencies between two populations. Here, we use upper case letters for populations ($A$ and $B$) and lower case letters for allele frequencies ($a$ and $b$).
...
\end{equation}
$$
If $f_4(A, B; C, D)$ is significantly different from zero, it suggests that $A$ and $B$ are not a clade relative to $C$ and $D$.
...
If $f_3(A; B, C)$ is negative, this suggests that $A$ is admixed between $B$ and $C$, or populations close to them.
...
Once finished, $f_2$-statistics for the populations of interest can be loaded like this.
...
If your data is so small that computing $f_2$-statistics doesn't take very long, you can also skip writing the data to disk and get the data in one step, by not providing an `outdir` argument:

```{r, eval = FALSE}
f2_blocks = extract_f2(my_f2_dir, pops = mypops)
```
...
The purpose of having separate estimates for each SNP block is to compute jackknife or bootstrap standard errors for any *f*-statistic, and for any other estimated parameter.
