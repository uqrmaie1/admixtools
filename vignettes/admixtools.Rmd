...
```{r, eval = FALSE}
f2_blocks = extract_data(prefix, pops = pops)
...
The function `fit_graph` generates and evaluates `numgraphs` admixture graphs in `numgen` iterations across `numrep` independent repeats to find well fitting admixture graphs.

```{r, eval = FALSE}
opt_results = fit_graph(my_f2_dir, pops, outpop = pops[1], numrep = 200,
...
With this setup the `fit_graph` function will submit each of the 200 repeats as a separate job.
...
opt_results = fit_graph('/my/f2/dir/', pops, outpop = pops[1], numrep = 200,
