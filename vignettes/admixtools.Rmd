...
The only requirements for the input graph are that the graph is a valid admixturegraph -- a DAG with one outgroup edge connecting the root to a leaf node -- and that f2-statistics exist for all leaf nodes. The first edge should go from the root to the outgroup.
...
rand_results = qpgraph(rand_graph, f2_dir = f2_dir, verbose = TRUE)
```
...
```
**Unless `outdir` is specified in these wrapper functions, files in the working directory may be overwritten!**
...
Currently the f2-statistics are not scaled, which may cause the absolute values to differ. However, the relative values should not be very different.
...
```{r, eval = FALSE}
precomp = qpgraph_precompute(pops, f2_blocks, block_lengths)
f3_jest = precomp[[1]]
ppinv = precomp[[2]]
```
<br>
