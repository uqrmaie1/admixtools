...
Optionally, fitted and estimated $f_4$-statistics and the worst residual z-score can be returned by setting `return_f4 = TRUE`. When `f2_blocks_test` is provided, an out-of-sample score is computed and returned as **score_test**.
...
The interactive version of this can be useful for finding out which edges or population pairs behave differently between two models

```{r, warning = FALSE}
plotly_comparison(qpg_results, qpgraph_ref_results)
```
<br>
