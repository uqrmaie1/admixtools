...
f2_blocks_v2 = f2_from_precomp(my_f2_dir, inds = inds, pops = pops)
```
...
inds = c('Chimp.REF', 'Altai_snpAD.DG', 'Vindija_snpAD.DG',
         ...
         'Denisova_snpAD.DG', 'UstIshim_snpAD.DG', 'Bichon.SG')
pops = c('Chimp', 'Neanderthal', 'Neanderthal',
         'Mbuti', 'Mbuti',
         'Denisova', 'Russia', 'Switzerland')
f2_blocks_v3 = f2_from_precomp(my_f2_dir, inds = inds, pops = pops)
```
...
group_samples(dir, mbuti, 'Mbuti')

inds = c('Chimp.REF', 'Altai_snpAD.DG', 'Vindija_snpAD.DG', 'Mbuti', 'Denisova_snpAD.DG', 'UstIshim_snpAD.DG', 'Bichon.SG')
f2_blocks_v4 = f2_from_precomp(dir, inds = inds)
```

Data from these groups can be deleted like this:

```{r, eval = FALSE}
delete_groups(dir, 'Mbuti')
```
...
```{r, eval = FALSE}
indres = qpgraph_resample_inds(dir, inds = inds, pops = pops, graph = example_graph)
```
