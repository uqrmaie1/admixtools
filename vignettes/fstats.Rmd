...
1. Using only SNPs present in all populations in a given analysis (`maxmiss = 0` / `allsnps: NO`)
2. Using only SNPs present in all four populations of any given $f_4$-statistic (`allsnps`)
3. Excluding SNPs or populations with high levels of missingness (`maxmiss > 0`)
4. Using allele frequency products rather than $f_2$-statistics to compute $f_4$-statistics (`afprod = TRUE`)
...
In practice it is generally not the case that small violations of the strict SNP ascertainment requirements affect the results in a substantial manner. SNP ascertainment is usually not random with respect to allele frequencies in any given population, nor with respect to allele frequency differences between two populations, but it is usually more or less random with respect to covariances among allele frequency differences. If there is doubt about this, it can help to repeat analyses under different SNP ascertainment schemes.
