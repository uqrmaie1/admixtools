...
The same `f4_var_rcond` and `f4_var_singular_loadings` diagnostics described above are surfaced on `qpadm_sweep()`'s output (`f4_var_rcond` unconditionally; `f4_var_singular_loadings` as a list-column when `full_results = TRUE`), making it straightforward to flag or filter rank-deficient fits across an entire sweep.
