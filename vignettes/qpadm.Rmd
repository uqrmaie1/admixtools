...
The theory above maps onto a single function call. With f-statistics in `example_f2_blocks`, we can ask whether the Pleistocene European `Switzerland_Bichon.SG` can be modeled as a mix of an Ust'-Ishim-like ancestor (`Russia_Ust_Ishim.DG`, an early modern Eurasian) plus Neanderthal admixture from `Vindija.DG`, with the remaining populations as outgroups:
...
The most-frequently-consumed pieces of the return value are the admixture weights and the rank-by-rank fit table:
...
- `weights$weight` are the estimated proportions of each source. `se` is the standard error; the row `weight / se = z` follows. For this fit, essentially all of Bichon's ancestry is modeled as Ust'-Ishim-like, with a small (statistically negligible) Vindija contribution — consistent with the published biology that the Neanderthal trickle into Pleistocene Europeans is largely already carried by the Ust'-Ishim-like ancestor in this set.
- `rankdrop$p[1]` is the *p*-value for the model at the auto-selected rank (the top row, `length(left) - 1`). A small *p* rejects the null hypothesis that the right populations are connected to `target` only through the sources. Here `p ≈ 0.07`: the 2-source model is not rejected at α = 0.05.
- `rankdrop$p_nested[1]` compares the full 2-source model to the simpler one-rank-lower model (the cladal alternative, where `target` is cladal with one of the sources). A small value rejects the cladal alternative in favor of the 2-source model. This is a structural test about model topology — distinct from the per-source weight z-scores. `p_nested` can reject the cladal alternative even when the second source's weight is itself statistically small (as in this fit, where `p_nested = 0` despite Vindija's `z = −1.75`); the f4 patterns simply don't fit the rank-0 topology.

When `data` is the prefix of a genotype file rather than precomputed f2-statistics, `qpadm()` reads from disk in parallel under any active `future::plan()` (see [the parallelization vignette](parallel.html)).


### Diagnosing collinear right populations

`qpadm()` inverts a square matrix of f4 covariances. When two right populations are close to cladal with each other — for example, a sister-clade pair, or one right population too close to a source — that matrix becomes near-singular, the inversion falls back to a pseudo-inverse, and the returned standard errors look misleadingly tight. The fitted weights themselves remain interpretable; the *uncertainty* on them does not.

`qpadm()` surfaces this with two diagnostics on the returned list:

- `res$f4_var_rcond` — the reciprocal condition number of the (fudged) f4 variance matrix. Values near machine epsilon (~`1e-15`) signal a degenerate variance matrix. A value below `1e-8` triggers an automatic warning.
- `res$f4_var_singular_loadings` — when `f4_var_rcond < 1e-8`, this tibble surfaces *which right populations* are responsible. Mechanically: the singular value decomposition of `f4_var` produces a right-singular vector for the smallest singular value — the direction in f4-space where the matrix has lost rank. That vector has one entry per `(left, right)` f4-pair; reshaping it to a `(R−1) × (L−1)` matrix and taking row L2 norms gives one loading per right population. Right pops at the top of the sorted tibble are the most-likely offenders — typically a sister-clade pair (two pops with similar high loadings) or a lone right pop too close to a source.

For pipelines that should fail loudly on degenerate inputs, the `singular_threshold` argument promotes the warning to an error:

```{r, eval = FALSE}
# Error out (rather than warn) if rcond < 1e-12
qpadm(data, left, right, target, singular_threshold = 1e-12)
```


### Automating the diagnosis: `qpadm_with_pruning`

Once a near-singular fit has been diagnosed, the hand-curation defense is the same loop every time: read the top of `f4_var_singular_loadings`, drop the offending right population, re-fit, and repeat until `f4_var_rcond` clears the concern bar. `qpadm_with_pruning()` codifies that loop:

```{r, eval = FALSE}
fit = qpadm_with_pruning(
  data, left, right, target,
  singular_threshold = 1e-12)   # convergence criterion (not a fail-loud gate)
fit$converged          # TRUE if rcond cleared the threshold
fit$pruning_trail      # tibble: iteration, dropped pop, loading, rcond
fit$right_final        # the converged right set
fit$reason             # "converged" / "floor" / "null_loadings" / ...
```

`min_right_pops` sets the smallest right set the loop will accept before giving up. It defaults to the structural minimum for identification (`length(left)` plus one when a `target` is supplied), so you only need to set it when you want a more conservative floor.

The returned object is the final `qpadm()` fit (so `f4`, `rankdrop`, `f4_var_rcond`, and `f4_var_singular_loadings` are present, along with `weights` and `popdrop` whenever a `target` is supplied) with four pruner-specific slots appended. The `pruning_trail` is the load-bearing audit surface: a downstream batch tool can scan the trail across many calls and notice that a particular reference-panel pop is consistently being pruned, which is itself a signal that the panel needs review.

Two strategies are available via the `strategy` argument:

- `"greedy"` (default) drops the single right pop with the largest absolute loading on the smallest right-singular vector. Cost: one `qpadm()` fit per iteration. Almost always correct.
- `"lookahead"` evaluates the top `lookahead_top_j` candidate drops at each step by actually performing the post-drop fit, then commits the candidate with the largest post-drop `f4_var_rcond`. Cost: `lookahead_top_j + 1` fits per iteration (e.g. 4x greedy at the default `lookahead_top_j = 3`). Worth the extra runtime when two right pops have near-equal loadings but very different post-drop rconds (the "sister-pair masking" edge case, where greedy's `which.max` would pick the wrong one and chain into over-pruning).

```{r, eval = FALSE}
# Greedy is fast and almost always right
qpadm_with_pruning(data, left, right, target)

# Lookahead trades ~4x runtime for robustness against sister-pair masking
qpadm_with_pruning(data, left, right, target,
                   strategy = "lookahead", lookahead_top_j = 3)
```

The pruner is greedy across iterations (it commits to a drop at each step and does not backtrack). For panels where a globally optimal subset matters more than runtime, the next escalation would be a full branch-and-bound enumeration; that is out of scope for the v1 function.

Defensive guards return early with `converged = FALSE` and a populated `reason` if the diagnostic surface returns something unexpected: an empty or `NULL` loadings table, a named drop pop that is not in the active right set, the floor `min_right_pops` being hit before convergence, or the iteration cap being reached.


### Multiple models: qpadm_multi and qpadm_sweep

`qpadm_multi()` runs a fixed list of `(left, right, target)` models, re-using f4-statistics where possible:
...
qpadm_multi(example_f2_blocks, models)
```

`qpadm_sweep()` covers the "many targets across a grid of source / outgroup configurations" question. Given vectors of `targets`, `source_sets`, and `right_sets`, it fits every combination in the Cartesian product and returns a flat tibble of *p*-values, weights, and `feasible` flags ready for filtering:
...
res %>% dplyr::arrange(target, p)
```

The same `f4_var_rcond` and `f4_var_singular_loadings` diagnostics described above are surfaced on `qpadm_sweep()`'s output (`f4_var_rcond` unconditionally; `f4_var_singular_loadings` as a list-column when `full_results = TRUE`), making it straightforward to flag or filter rank-deficient fits across an entire sweep. The loadings tibble fires per row when `f4_var_rcond < 1e-8` (the auto-bar) and is `NULL` otherwise. Note that passing `singular_threshold` via `...` to `qpadm_sweep()` is not the "mark and continue" path callers might expect; `qpadm()` raises an error when the threshold trips and the sweep aborts. For sweep-wide rank-deficiency triage, omit `singular_threshold` and filter on the returned `f4_var_rcond` column instead.

Both functions parallelize across models under `future::plan(multisession)` (or `multicore` on Linux). Be aware that the per-block layer inside each model also parallelizes when `data` is a genotype-file prefix, so two layers compose — usually you want only one of them parallel.


### qpwave-style runs against genotype data

When you only care about the rank of $X$ and not about admixture weights for a specific target, the qpwave-on-genotype-data pattern is `qpadm(..., target = NULL, return_f4 = TRUE)`:
...
This shape is useful in sweep-style workflows where the rank-only question is one of many you're asking against the same genotype data — you get the same parallel f4 read for free.
