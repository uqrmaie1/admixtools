...
In the graph below, the path between $C$ and $D$ is composed of only two segments (one unique to $C$ and one unique to $D$), while the path between $B$ and $D$ is composed of three segments, and the middle segment ($n1|n2$) is also part of paths connecting other population pairs, for example the path from $B$ to $C$.
(Hovering over the graph will show the names of the internal nodes.)
...
example_f2sim1 = f2_from_msprime(g, nsnps = 1e5, run = '/Users/robert/anaconda2/bin/python')
dates = pseudo_dates(g, 1000)
dates['C'] = 0
example_f2sim2 = f2_from_msprime(g, nsnps = 1e5, run = '/Users/robert/anaconda2/bin/python', time = dates)
```
