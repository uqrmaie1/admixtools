...
```{r}
my_graph_function = graph_f2_function(g, admix_default = 0.5, drift_default = 0.1)
my_graph_function(`a|A` = 0) %>% head
my_graph_function(`a|A` = 0.2) %>% head
```

Similarly, we can see how $f_4$-statistics are affected by changing one of the drift weights:

```{r}
my_graph_function(`a|A` = 0) %>% f2dat_f4dat() %>% head
my_graph_function(`a|A` = 0.2) %>% f2dat_f4dat() %>% head
```


Usually we observe *f*-statistics and want to estimate drift and admixture weights. That's the inverse of the function above, and it is what `qpgraph()` does.
...
Filtering lists of graphs and generating random graphs meeting these constraints work analogously to the example above.
