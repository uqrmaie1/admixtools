...
The `qpgraph()` function tries to find the admixture- and drift-edge weights for a given graph topology which are most consistent with the observed $f_3$-statistics. The optimization of admixture weights requires initial values. Occasionally, these initial values have an effect on the final weight estimates because the optimization gets stuck in a local optimum. Because of this, the `qpgraph()` weight optimization is repeated by default with 10 different randomly chosen combinations of starting weights, and the final likelihood score represents the best score out of these 10 tries. The default value of 10 can be decreased to speed up `qpgraph()`, or increased to reduce the risk of missing the global optimum.
...
The range of estimated weights for each admixture edge is also stored in the `low` and `high` columns of the `edges` data frame returned by `qpgraph()`. The `plot_comparison()` function visualizes that information with grey error bars.
...
There are two regularization terms (fudge factors) used in qpGraph, which make matrix inversions more stable.
One is added to the diagonal elements of the $f_3$ covariance matrix (after multiplying it by the mean of the diagonal elements). It is called `fudge_cov` and defaults to 10^-5^. Increasing this factor has a small effect on admixture weights and branch lengths, but will shift the likelihood scores to be *closer to zero*.

The other regularization terms is added to the covariance matrix of fitted branch lengths (after scaling as before). It is called `fudge` and defaults to 10^-4^. In the original *qpGraph* program, this parameter can be set through the `diag` parameter. Increasing `fudge` will shift the likelihood scores to be *further away from zero*. This regularization term ensures that edge weights will be evenly distributed across a set of edges whose weights could otherwise not be determined unambiguously. (As is often the case with the two edges originating in the root node.)

As these regularization terms affect the likelihood score, they should be kept constant whenever fits of different graphs are compared to each other.
