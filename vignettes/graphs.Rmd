...
An admixture graph can be seen as a function which maps drift and admixture weights to a set of *f*-statistics. How it does that depends on its topology which is captured by its equation system.
We can create this function for any given graph, and use it see how different edge weights lead to different *f*-statistics. 
...
Each admixture graph can be described by an equation system which relates drift and admixture weights to *f*-statistics. This equation system may be unidentifiable, which means it is not possible to estimate all the weights at the same time. (In practice we will still get estimates for these weights because of the regularization terms; but these estimates don't mean much.)
...
```{r, echo = F, out.width="100%"}
matrix(c('R', 'A', 'R', 'n0', 'n0', 'n', 'n', 'n1', 'n', 'n2', 'n1', 'C', 'n1', 'n2', 'n2', 'D', 'n0', 'B'), , 2, byrow = T) %>% plotly_graph
```
...
```{r, echo = F, out.width="100%"}
matrix(c('R', 'A', 'R', 'n0', 'n0', 'n', 'n', 'n1', 'n', 'n2', 'n1', 'n3', 'n3', 'C', 'n1', 'n2',
         'n2', 'n4', 'n4', 'D', 'n2', 'n4', 'n3', 'n4', 'n0', 'B'), , 2, byrow = T) %>% plotly_graph
```
...
On the other hand, this example shows that even though admixture graphs have only pulse admixture events and no continues admixture or split events, the fit of these highly discretized models is identical to a much larger class of models with continuous admixture and population splits.
