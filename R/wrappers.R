...
#' @export
#' @param graph An admixture graph
#' @param min Lower and upper limit of uniform distribution
#' @param max Upper limit of uniform distribution
#' @param fix Boolean specifying if leaf nodes will be fixed to 0. If `TRUE` (default), all samples will be drawn at the end of the simulation (i.e., from today).
#' @return A named vector with random dates for each graph node
random_dates = function(graph, min=1000, max=1000, fix=TRUE){
  edges = graph %>% as_edgelist()
  pdat = graph_to_plotdat(edges)$eg
...
#' @param recomb_rate_chr A float value specifying recombination rate along the chromosomes
#' @param seq_length Either a scalar value specifying the length of all chromosomes or a vector with a length equal to number of chromosomes.
#' @param admix_default The default admixture proportions for all admixture nodes
#' @param run If `FALSE`, the function will terminate after writing the msprime script. If `TRUE`, it will try and execute the function with the default python installation. If you want to use some other python installation, you can set `run = /my/python`.
#' @param ghost_lineages If `TRUE` (default), admixture events occur at time points shown on admixture graph (assumes ghost lineages) by \code{\link{plot_graph}}
#' @param shorten_admixed_leaves If `TRUE` simulate the behavior of treemix where drift after admixture is not allowed
...
#'
#' This function first generates a random graph that satisfy the `max_depth` constrain using \code{\link{random_admixturegraph}.
#' Then it simulates a genome using \code{\link{msprime_genome}
#' 
#' @export
#' @param nlaef Number of leaf nodes (i.e., number of final populations)
...
#' @param ind_per_pop The number of individuals to simulate for each population. If a scalar value, it will be constant across all populations. Alternatively, it can be a named vector with a different value for each population.
#' @param mutation_rate Mutation rate per site per generation. The default is set to a high value (0.001) to obtain more polymorphic SNPs in order to speed up the simulation.
#' @param admix_weights Admixture weights. If a float value (0 < value < 1), admixture weights for each admixture edge will be (value, 1-value). Alternatively, it can be a range, i.e., c(0.1, 0.4) specifying lower and upper limits of an uniform distribution from which admixture weight value will be drawn. By default, all admixture edges will have weight 0.5. 
#' @param neff Effective population size. If a scalar value, it will be constant across all populations. Alternatively, it can be a range, i.e., c(500, 1000) specifying lower and upper limits of an uniform distribution from which values will be drawn 
#' @param time Time between nodes. Either a scalar value (1000 by default) which the dates generated by \code{\link{pseudo_dates}}, or a range, i.e., c(500, 1000) specifying lower and upper limits of an uniform distribution from which values will be drawn (see \code{\link{pseudo_dates}}). 
#' @param fix_leaf Boolean specifying if leaf nodes will be fixed to 0. If TRUE, all samples will be drawn at the end of the simulation (i.e., from today).
#' @param outpop Name of the outgroup population. 
#' @param nchr Number of chromosomes to simulate
#' @param recomb_rate A float value specifying recombination rate along the chromosomes.
#' @param seq_length The sequence length of the chromosomes
#' @param ghost_lineages If `TRUE` (default), admixture events occur at time points shown on admixture graph (assumes ghost lineages) by \code{\link{plot_graph}}
#' @param run If `FALSE`, the function will terminate after writing the msprime script. If `TRUE`, it will try and execute the function with the default python installation. If you want to use some other python installation, you can set `run = /my/python`.
...
random_sim = function(nleaf, nadmix, outpref = "random_sim", max_depth = NULL, ind_per_pop = 1, mutation_rate = 1e-3, admix_weights = 0.5, neff = 1000,
                      time = 1000, fix_leaf = FALSE, outpop = NULL, nchr = 1, recomb_rate = 1e-8, seq_length = 1000, ghost_lineages = TRUE, run = FALSE){
