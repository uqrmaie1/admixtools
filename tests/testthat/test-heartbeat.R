...
#   Non-TTY (`.tty = FALSE`) -> cli `i` message per emit, no-op on done = TRUE.
#
...
test_that(".heartbeat non-TTY mode emits a cli i-bulleted message", {
  # The orchestrator-tail contract: each emit lands on stderr as its own
  # newline-terminated line, with cli's i bullet (U+2139) and no ANSI
  ...
  expect_length(out, 1)
  # Bullet glyph + space + msg + \n. The glyph is `i` (U+2139) when cli
