...
Running `devtools::install_github("uqrmaie1/admixtools")` will compile C++ from source code (this isn't necessary when installing packages from CRAN with `install.packages()`). On a build with OpenMP enabled, the compiled library stays loaded for the rest of the R session, so if you recompile *ADMIXTOOLS 2* in the same session you need to restart R to pick up the new build.

If you get the following error:

`Error: Failed to install 'admixtools' from GitHub:  Could not find tools necessary to compile a package.`

you might be able to solve the problem by installing [Rtools](https://cran.r-project.org/bin/windows/Rtools/) for your version of R if you use Windows, or [Xcode command line tools](https://developer.apple.com/download/all/) if you use macOS. This is also described [here](https://r-pkgs.org/setup.html#setup-tools).
...
* [MixMapper](https://cb.csail.mit.edu/mixmapper/) Another method to infer admixture graphs
* [TreeMix](https://bitbucket.org/nygcresearch/treemix/wiki/Home) Another method to infer admixture graphs
* [Legofit](https://github.com/alanrogers/legofit) A program to estimate the history of population size, subdivision, and gene flow
