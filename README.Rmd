...
*ADMIXTOOLS 2* provides the same functionality in a new look, and it's orders of magnitude faster. This is achieved through separating the computation of $f_2$-statistics from all other computations, and through a number of other optimizations. In the example below, rendering the plot takes much longer than computing the fit of a new *qpGraph* model:
